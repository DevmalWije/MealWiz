        ...
        GlobalScope.launch {
            ...
            if (meals.size > 0) {
                ...
                for (i in 0 until meals.size) {
                    ...
                    Handler(Looper.getMainLooper()).post {
                        Toast.makeText(context, "Meals added to database", Toast.LENGTH_SHORT).show()
                    }
                }
            }else{
                Handler(Looper.getMainLooper()).post {
                    Toast.makeText(context, "No meals found matching your query", Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
